          ...
          {Array.from({ length: 8 }).map((_, rowIndex) => (
            <div key={rowIndex} className="row flex flex-1 w-full ">
              {Array.from({ length: 16 }).map((_, blockIndex) => (
                <M.div
                  key={blockIndex}
                  className="block relative flex-1 bg-black/90 m-[-0.25px] origin-top inset-shadow-xl"
        ...
        {Array.from({ length: 8 }).map((_, rowIndex) => (
          <div key={rowIndex} className="row flex flex-1 w-full">
            {Array.from({ length: 16 }).map((_, blockIndex) => (
              <M.div
                key={blockIndex}
                className="block relative flex-1 bg-black/90 m-[-0.25px] origin-bottom inset-shadow-xl"
